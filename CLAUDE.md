...
Turn any API into a webhook. See `VISION.md` for architecture and `ARTICLE.md` for the first thought piece that talked about what I want to accomplish (the syntax in the article is aspirational, not necessarily exact).
...
2. Create a worktree: `git worktree add ../lampas-<N> -b issue-<N>`
...
gh api repos/mikesol/lampas/pulls/<N>/comments --jq '.[] | {id, user: .user.login, path, line, body: .body[:80]}'

# Reply to a review comment (in_reply_to creates a thread reply)
gh api repos/mikesol/lampas/pulls/<N>/comments -f body="Your reply" -F in_reply_to=<comment_id>

# Get thread IDs for resolving
gh api graphql -f query='{ repository(owner: "mikesol", name: "lampas") { pullRequest(number: <N>) { reviewThreads(first: 50) { nodes { id isResolved } } } } }'
...
- **Keep files under 300 lines.** Source files and test files must stay under 300 lines. If a file would exceed this, split it into focused modules with clear imports. Test files should mirror source structure — one test file per source module. This limit will be enforced by biome (see issue tracker).
